...
class APIUtils:   
    ...
    def get_keyword_and_keys_semantic_similarity(keywords_list: list, json_object):
        ...
        key_set = set()
        APIUtils.get_json_keys(json_object, key_set)
        list_of_keys = list(key_set)

        # Cria um dicionário com os synsets correspondentes a cada chave do objeto JSON
        keys_synsets_dict = APIUtils.set_keywords_synsets_dict(list_of_keys)

        # Cria um dicionário com os synsets correspondentes a cada palavra-chave da lista
        keywords_synsets_dict = APIUtils.set_keywords_synsets_dict(keywords_list)

        # Combina os synsets dos dois dicionários e calcula a similaridade entre eles
        return APIUtils.combine_synsets_and_map_similarity(keywords_synsets_dict, keys_synsets_dict)
    ...
    def extrair_info_api_por_chaves_similares(
            ...
            for chave, valor in json_api.items():
                for chave_combinada in dict_palavras_chave.keys():
                    lista_palavras_combinadas = chave_combinada.split("_")
                    ...
                    if chave in lista_palavras_combinadas:
                        ...
                        # Trata os dicionários aninhados, chamando a função recursivamente
                            ...
                            etiqueta = prefixo_rotulo + chave
                            tupla_itens_dict = APIUtils.extrair_info_api_por_chaves_similares(
                                etiqueta + "_", 
                                valor, 
                                dict_palavras_chave, 
                                store_id, 
                            ...
                            if store_id is not None:
                                dict_exportar.update({'store_id': store_id, 'data_extracao_loja_utc': data_extracao})
                            else:
                                dict_exportar.update({'data_extracao_loja_utc': data_extracao})
                        ...
                        # Trata listas de valores
                            ...
                            for item in valor:
                                ...
                                if isinstance(item, dict):
                                    ...
                                    tupla_info_itens = APIUtils.extrair_info_api_por_chaves_similares(
                                        etiqueta, 
                                        item, 
                                        dict_palavras_chave, 
                                        store_id, 
                                ...
                                elif isinstance(valor, (int, str, float, bool)):
    ...
    @staticmethod
    def get_info_store_ifood(unique_df_info_lojas_ifood):
        headers = {
        ...
        for store_delivery_id in tqdm(unique_df_info_lojas_ifood['store_delivery_id'].unique()):
            ...
            if response.status_code == 200:
                data = response.json()
                dict_region_info = data['data']['merchantExtra']['address']
                dict_region_info.update({'store_delivery_id': store_delivery_id})
                df_region_info = pd.concat([df_region_info, pd.DataFrame.from_dict(dict_region_info, orient='index').T], ignore_index=True)
            else:
                print(f'Erro: {response.status_code} - {response.reason}')
            
        df_info_store_ifood = unique_df_info_lojas_ifood.merge(df_region_info, on='store_delivery_id', how='left')
        del df_region_info
        df_info_store_ifood = df_info_store_ifood.rename(columns={
                                                'data_extracao_loja_utc': 'store_delivery_date_utc',
                                                'city': 'store_delivery_city', 
                                                'country': 'store_delivery_contry',
                                                'district' : 'store_delivery_district',
                                                'latitude' : 'store_delivery_latitude',
                                                'longitude' : 'store_delivery_longitude',
                                                'state' : 'store_delivery_state',
                                                'streetName' : 'store_delivery_streetName',
                                                'streetNumber' : 'store_delivery_streetNumber',
                                                'zipCode': 'store_delivery_zipCode'
                                            }).drop(columns='store_delivery_date_utc')
        
        return df_info_store_ifood
