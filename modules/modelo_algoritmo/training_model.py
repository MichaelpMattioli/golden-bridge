...
class RidgeModelMultiTarget:
    ...
    def ridge_model_multitarget(self, data, feature_names, target_names, shuffle, metrics=None):
        ...
        for train_index, val_index in kfold.split(X_train_val_scaled):
            ...
            for _ in range(self.n_bootstrap):
                ...
                score = r2_score(y_val, y_pred_resampled, multioutput="uniform_average")
                ...
                for metric in metrics:
                    score = metric(y_val, y_pred_val, multioutput="uniform_average")
        ...
        test_scores = {
            f"Test {metric.__name__}": metric(
                y_test, y_pred_test, multioutput="uniform_average"
            )
            for metric in metrics
        }
        ...
        metrics = {
            ...
            "Std CV Scores": {
                metric: np.std(scores) for metric, scores in cv_scores.items()
            },
    ...
    def train_model_and_get_errors(self):
        ...
        # if metrics['Mean CV Scores']['r2_score'] < self.r2_score_minimo:
        if max(abs(error["residuo_net_sale%"])) > self.residual_max: print("Treinamento ruim, escolha um período maior")
        else:
            print(50 * "-")
            # Imprime os valores mínimos e máximos de resíduos e resíduos percentuais
            ...
            # print('CV Scores:' ,metrics['CV Scores'])
            print(50 * "-")
            print("Mean CV Scores:", metrics["Mean CV Scores"])
            print("Std CV Scores:", metrics["Std CV Scores"])
            print("Test Scores:", metrics["Test Scores"])
