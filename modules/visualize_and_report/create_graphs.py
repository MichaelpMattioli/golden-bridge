...
class ModelAnalysis:
    def __init__(
        self, X_weighted, feature_names, all_data_pred_sum_plu_net_sale_calculado, 
        df_model, produto, clima, compt, camp, demo, flag ,lojas, data_1, data_2
        ):
        self.flag = flag
        self.lojas = [lojas]
        ...
        self.data_inicio1 = '2022-01-01' #  2020-01-01
        self.data_fim1 = '2022-01-31' #     2020-03-31      2020-01-31
        self.data_inicio2 = '2022-04-01' #  2020-01-31      2020-04-01
        self.data_fim2 = '2022-04-30' #     2020-06-30      2020-11-30
        ...
        # Plots do período
        # Net Sale
        self.plot_net_sale_geral = self.plot_peso_var_stack("Influência dos Pilares Diariamente<br>Net Sale", *[self.df_dist_perc_net_sale.filter(regex="PRODUTO").filter(regex="sold"), self.df_dist_perc_net_sale.filter(regex="DEMO"), self.df_dist_perc_net_sale.filter(regex="CLIMA"), self.df_dist_perc_net_sale.filter(regex="COMPT"), self.df_dist_perc_net_sale.filter(regex="CAMP"), self.df_dist_perc_net_sale.filter(regex="PRODUTO").filter(regex="tax|imposto"), self.df_dist_perc_net_sale.filter(regex="PRODUTO").filter(regex="cost"), self.df_dist_perc_net_sale.filter(regex="PRODUTO").filter(regex="price")])
        
        # Margem
        self.plot_margem_geral = self.plot_peso_var_stack("Influência dos Pilares Diariamente<br>Margem", *[self.df_dist_perc_margem.filter(regex="PRODUTO").filter(regex="sold"), self.df_dist_perc_margem.filter(regex="DEMO"), self.df_dist_perc_margem.filter(regex="CLIMA"), self.df_dist_perc_margem.filter(regex="COMPT"), self.df_dist_perc_net_sale.filter(regex="CAMP"), self.df_dist_perc_margem.filter(regex="PRODUTO").filter(regex="tax|imposto"), self.df_dist_perc_margem.filter(regex="PRODUTO").filter(regex="cost"), self.df_dist_perc_margem.filter(regex="PRODUTO").filter(regex="price")])
        
        # GC
        self.plot_gc_geral = self.plot_peso_var_stack("Influência dos Pilares Diariamente<br>GC", *[self.df_dist_perc_gc.filter(regex="PRODUTO").filter(regex="sold"), self.df_dist_perc_gc.filter(regex="DEMO"), self.df_dist_perc_gc.filter(regex="CLIMA"), self.df_dist_perc_gc.filter(regex="COMPT"), self.df_dist_perc_net_sale.filter(regex="CAMP"), self.df_dist_perc_gc.filter(regex="PRODUTO").filter(regex="tax|imposto"), self.df_dist_perc_gc.filter(regex="PRODUTO").filter(regex="cost"), self.df_dist_perc_gc.filter(regex="PRODUTO").filter(regex="price")])
        ...
        self.plot_net_sale_camp = self.plot_peso_var(self.df_dist_perc_net_sale, "CAMP", "Campanha")
        ...
        self.plot_margem_camp = self.plot_peso_var(self.df_dist_perc_margem, "CAMP", "Campanha")
        ...
        self.plot_gc_camp = self.plot_peso_var(self.df_dist_perc_gc, "CAMP", "Campanha")
    ...
    def get_coeficients(self):
        ...
        self.dados_pilares_1 = [
            self.df_net_sales.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_precos.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_campanhas.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_novos_produtos.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_novas_lojas.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_crm.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_delivery.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_cmv.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_custos.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_taxas.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_competidores.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_clima.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_demo.loc[self.data_inicio1:self.data_fim1].mean(),
            self.df_coef_net_sales_qtd_vendidas.loc[self.data_inicio1:self.data_fim1].mean(),
        ]
        

        self.dados_pilares_2 = [
            self.df_net_sales.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_precos.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_campanhas.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_novos_produtos.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_novas_lojas.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_crm.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_delivery.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_cmv.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_custos.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_taxas.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_competidores.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_clima.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_demo.loc[self.data_inicio2:self.data_fim2].mean(),
            self.df_coef_net_sales_qtd_vendidas.loc[self.data_inicio2:self.data_fim2].mean(),
        ]

    def plot_cascata_diferenca(
        self, target, df_target, df_coef_target_clima, df_coef_target_demo, df_coef_target_qtd_vendidas, 
        df_coef_target_precos, df_coef_target_taxas, df_coef_target_custos, df_coef_target_competidores, 
        df_coef_target_campanhas, df_coef_target_novos_produtos, df_coef_target_crm, df_coef_target_novas_lojas, 
        df_coef_target_delivery, df_coef_target_cmv
        ):
        ...
        if self.flag == 1:
            ...
            dados_diferenca = [
                diferenca_bruta(df_coef_target_precos.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_precos.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_bruta(df_coef_target_campanhas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_campanhas.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_bruta(df_coef_target_novos_produtos.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_novos_produtos.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_bruta(df_coef_target_novas_lojas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_novas_lojas.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_bruta(df_coef_target_crm.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_crm.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_bruta(df_coef_target_delivery.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_delivery.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_bruta(df_coef_target_cmv.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_cmv.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_bruta(df_coef_target_custos.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_custos.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_bruta(df_coef_target_taxas.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_taxas.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_bruta(df_coef_target_competidores.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_competidores.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_bruta(df_coef_target_clima.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_clima.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_bruta(df_coef_target_demo.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_demo.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_bruta(df_coef_target_qtd_vendidas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_qtd_vendidas.loc[self.data_inicio1:self.data_fim1].mean(),),
            ]
            ...
            valores_y = [df_target.loc[self.data_inicio1:self.data_fim1].mean()] + dados_diferenca_ + [df_target.loc[self.data_inicio2:self.data_fim2].mean()]

        else:
            # Calcule a diferença entre os dados para as duas datas
            dados_diferenca = [
                diferenca_percentual(df_coef_target_precos.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_precos.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_percentual(df_coef_target_campanhas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_campanhas.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_percentual(df_coef_target_novos_produtos.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_novos_produtos.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_percentual(df_coef_target_novas_lojas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_novas_lojas.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_percentual(df_coef_target_crm.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_crm.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_percentual(df_coef_target_delivery.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_delivery.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_percentual(df_coef_target_cmv.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_cmv.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_percentual(df_coef_target_custos.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_custos.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_percentual(df_coef_target_taxas.loc[self.data_inicio2:self.data_fim2].sum(), df_coef_target_taxas.loc[self.data_inicio1:self.data_fim1].sum()),
                diferenca_percentual(df_coef_target_competidores.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_competidores.loc[self.data_inicio1:self.data_fim1].mean(),),
                diferenca_percentual(df_coef_target_clima.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_clima.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_percentual(df_coef_target_demo.loc[self.data_inicio2:self.data_fim2].mean(), df_coef_target_demo.loc[self.data_inicio1:self.data_fim1].mean()),
                diferenca_percentual(df_coef_target_qtd_vendidas.loc[self.data_inicio2:self.data_fim2].mean(),df_coef_target_qtd_vendidas.loc[self.data_inicio1:self.data_fim1].mean(),),
            ]
            # pilares_, dados_diferenca_ = sort_self.pilares(self.pilares, dados_diferenca)
            pilares_, dados_diferenca_ = self.pilares, dados_diferenca
            val_ini = [100]
            if target == "net_sales":
                val_fin = [df_target.loc[self.data_inicio2:self.data_fim2].sum() * 100 / df_target.loc[self.data_inicio1:self.data_fim1].sum()]
            elif target == "margem":
                val_fin = [df_target.loc[self.data_inicio2:self.data_fim2].mean() * 100 / df_target.loc[self.data_inicio1:self.data_fim1].mean()]
            elif target == "gc":
                val_fin = [df_target.loc[self.data_inicio2:self.data_fim2].sum() * 100 / df_target.loc[self.data_inicio1:self.data_fim1].sum()]
            # val_fin = [df_target.loc[self.data_inicio2:self.data_fim2].mean() * 100 / df_target.loc[self.data_inicio1:self.data_fim1].mean()]
        ...
        if target == "net_sales":
            ...
            valores_txt = f"Bruto: {round(df_target.loc[self.data_inicio1:self.data_fim1].sum()/1000,2)}k e {round(df_target.loc[self.data_inicio2:self.data_fim2].sum()/1000,2)}k"
        elif target == "margem":
            target_txt = "MARGEM "
            valores_txt = f" {round(df_target.loc[self.data_inicio1:self.data_fim1].mean()*100,2)}% e {round(df_target.loc[self.data_inicio2:self.data_fim2].mean()*100,2)}%"
        elif target == "gc":
            target_txt = "GC "
            valores_txt = f"Bruto: {round(df_target.loc[self.data_inicio1:self.data_fim1].sum()/1000,2)}k e {round(df_target.loc[self.data_inicio2:self.data_fim2].sum()/1000,2)}k"
        ...
        loja_txt = (
            f"Filtro: {self.lojas[0]}"
            if len(self.lojas) == 1
            else "Filtro: " + ", ".join(map(str, self.lojas))
        )
        title_graph = f"Golden Bridge {target_txt}<br>Período Inicial:{self.data_inicio1} à {self.data_fim1} e Período Final:{self.data_inicio2} à {self.data_fim2}<br>{loja_txt}<br>Proporção percentual"
        fig.update_layout(
            title=dict(
    ...
    def generate_report(self, column):
        ...
        df1 = produto_familia_data.loc[self.data_inicio1:self.data_fim1].reset_index().groupby(by='PRODUTO_plus__p_familia_brasil').sum()
        df2 = produto_familia_data.loc[self.data_inicio2:self.data_fim2].reset_index().groupby(by='PRODUTO_plus__p_familia_brasil').sum()
        ...
        df_merge_diff = df_merge_diff.rename(
            columns={
                f"{column}_x": f"{column}_{self.data_inicio1}_{self.data_fim1}",
                f"{column}_y": f"{column}_{self.data_inicio2}_{self.data_fim2}",
                f"{column}": "Difference",
            }
        )
        
        df_merge_diff = df_merge_diff.drop(columns=df_merge_diff.filter(regex='calendario__d_date').columns)
        
        # Calcula a diferença percentual
        df_merge_diff["Difference_%"] = (
            df_merge_diff["Difference"] / df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"] * 100
        )
        
        # Preenche os valores NaN com 0
        df_merge_diff = df_merge_diff.fillna(0)
        
        # Preenche os zeros das diferenças inexistentes
        df_merge_diff.loc[(df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"] > 0) & (df_merge_diff[f"{column}_{self.data_inicio2}_{self.data_fim2}"] == 0), 'Difference'] = -df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"]
        df_merge_diff.loc[(df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"] > 0) & (df_merge_diff[f"{column}_{self.data_inicio2}_{self.data_fim2}"] == 0), 'Difference_%'] = -100
        df_merge_diff.loc[(df_merge_diff[f"{column}_{self.data_inicio2}_{self.data_fim2}"] > 0) & (df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"] == 0), 'Difference'] = df_merge_diff[f"{column}_{self.data_inicio2}_{self.data_fim2}"]
        df_merge_diff.loc[(df_merge_diff[f"{column}_{self.data_inicio2}_{self.data_fim2}"] > 0) & (df_merge_diff[f"{column}_{self.data_inicio1}_{self.data_fim1}"] == 0), 'Difference_%'] = 100
        
        # Ordena os dados pela diferença percentual
        df_merge_diff = df_merge_diff.sort_values("Difference_%", ascending = False)        
    ...
    def f_report_clima(self):
        ...
        df1 = self.clima.reset_index().set_index('calendario__d_date').loc[self.data_inicio1:self.data_fim1].reset_index().groupby(by='filtros_usados').mean().reset_index()
        df2 = self.clima.reset_index().set_index('calendario__d_date').loc[self.data_inicio2:self.data_fim2].reset_index().groupby(by='filtros_usados').mean().reset_index()
        
        # Calcula a diferença entre os dois dataframes
        df_report_clima = (
            df2.reset_index(drop=True).filter(regex='CLIMA').sub(df1.reset_index(drop=True).filter(regex='CLIMA'))
        )
